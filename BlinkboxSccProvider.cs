        ...
        private void OnBlinkboxCommitAndTest(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(commit.Id))
            {
                try
                {
                    ...
                    // open website
                    //// this.LaunchBrowser(launchUrls.FirstOrDefault()); // VS browser seems to regularly log you out - so launch in default browser. 
                    if (!string.IsNullOrEmpty(launchUrls.FirstOrDefault()))
                    {
                        System.Diagnostics.Process.Start(launchUrls.First());
                    }
                }
                catch (Exception exc)
                {
